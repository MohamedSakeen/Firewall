...
def evaluate_stateful_packet(packet):
    if packet.get("protocol") != "TCP":
        return "ALLOW"

    session_key = generate_session_key(packet)
    session = get_session_state(session_key)
    state = determine_state(packet.get("tcp_flags", ""))

    if not session:
        create_session(session_key, state)
        return "NEW_SESSION"

    update_session(session_key, state)
